...
printf '20 variables, 91 clauses - 1000 instances, all SATISFIABLE\n'
printf '\n'
...
do	
	RES="SATISFIABLE"
	RES_OUR=$(../../../DPLL ./data/uf20-91/uf20-0${test_idx}.cnf)

	if [[ $RES == $RES_OUR ]]; then	
		count_ok=$((count_ok+1))
	else
		failed_tests=$failed_tests" \tuf20-0${test_idx}.cnf\n"	
	fi
