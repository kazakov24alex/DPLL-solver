...
printf '50 variables, 218 clauses - 1000 instances, all UNSATISFIABLE\n'
printf '\n'
...
do	
	RES="UNSATISFIABLE"
	RES_OUR=$(../../../DPLL ./data/uf50-218/uuf50-0${test_idx}.cnf)

	if [[ $RES == $RES_OUR ]]; then	
		count_ok=$((count_ok+1))
	else
		failed_tests=$failed_tests" \tuf50-0${test_idx}.cnf\n"	
	fi
