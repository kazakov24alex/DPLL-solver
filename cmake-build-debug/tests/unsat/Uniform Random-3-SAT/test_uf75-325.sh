...
printf '75 variables, 325 clauses - 100 instances, all UNSATISFIABLE\n'
printf '\n'
...
do	
	RES="UNSATISFIABLE"
	RES_OUR=$(../../../DPLL ./data/uf75-325/uuf75-0${test_idx}.cnf)

	if [[ $RES == $RES_OUR ]]; then	
		count_ok=$((count_ok+1))
	else
		failed_tests=$failed_tests" \tuf75-0${test_idx}.cnf\n"	
	fi
