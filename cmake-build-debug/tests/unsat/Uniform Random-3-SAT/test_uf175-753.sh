...
printf '150 variables, 645 clauses - 100 instances, all UNSATISFIABLE\n'
printf '\n'
...
do	
	RES="UNSATISFIABLE"
	RES_OUR=$(../../../DPLL ./data/uf175-753/uuf175-0${test_idx}.cnf)

	if [[ $RES == $RES_OUR ]]; then	
		count_ok=$((count_ok+1))
	else
		failed_tests=$failed_tests" \tuf175-0${test_idx}.cnf\n"	
	fi
