...
printf '100 variables, 430 clauses - 1000 instances, all UNSATISFIABLE\n'
printf '\n'
...
do	
	RES="UNSATISFIABLE"
	RES_OUR=$(../../../DPLL ./data/uf100-430/uuf100-0${test_idx}.cnf)

	if [[ $RES == $RES_OUR ]]; then	
		count_ok=$((count_ok+1))
	else
		failed_tests=$failed_tests" \tuf100-0${test_idx}.cnf\n"	
	fi
